...
import matplotlib.pyplot as plt
 #extract the data provided by Indygo.
def extract_data():
    li = []  # to store all the data from the files.
    busid = []
    check = True # to store the bus Ids
    ...
    for filename in all_files:
        with open(filename) as file:
            ...
            check = False
            ##########################################################
            # read csv fine into a data frame
            df = pd.read_csv(filename, index_col = 0, header = 0)
            li.append(df) #append the dataframe in list for ever =y file
            file.close()
    #print(df.dtypes)
    #print(df.index)
    #print(df.columns)
    ...
    return(li)
        #data = pd.read_csv(filename, index_col = 0, header = 0) # store the data.
    #    li.append(data) # each element relates to each days data
        #bus.append()
        #py.arange()
    #    bus.
    #    bus_data = pd.concat(li) #all data under one header
    #print(bus)
    #return(li)
# stores the busIDS
...
def sort():
    ...
    for i in bus:
        for j in all_data:
            if j.index[a] == i:
                bus_df.append(j.iloc[a])

    if all_data[1].index[0] == bus[1]:
        print('YES')
    return()



def filterData(bus): #elimate outliers using 3-sigma
...
#buses.append(bus27)
#print(buses[1])
# for loops to get the average value for the distance travelled & Energy used
#    avrge_travel = buses[b]['Actual Distance'].mean()
#    print(f'{avrge_travel:.2f}')
#    avgDist.append(avrge_travel)
#print(f'-----------------------')
#for b in range(27):
#    avrgeEnrgy = buses[b]['Energy Used'].mean()
#    print(f'{avrgeEnrgy:.2f}')
#    avgEused.append(avrgeEnrgy)

#plotting
#fig, ax = plt.subplots()
#
#ax.scatter(buses['kWh/Miles'],bus_ID)
